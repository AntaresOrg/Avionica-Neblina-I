                                                    ...
                                                    float current_altitude_m)
{
    ...
    if (altitude_is_valid(current_altitude_m))
    {
        ...
        if (!controller->reef_deployed && current_altitude_m >= controller->thresholds.target_altitude_m)
            controller->reef_deployed = true;

        if (controller->reef_deployed && !controller->chute_deployed &&
    ...
    if (!controller->reef_deployed && resolved_altitude_m < controller->thresholds.ground_altitude_m)
    {
        controller->phase = FLIGHT_STATE_GROUND;
    }
    else if (controller->reef_deployed && controller->chute_deployed &&
                                               ...
                                               size_t out_len)
{
    ...
    int written = snprintf(out,
                           out_len,
                           "STATE=%s,ALT=%.2f,REEF=%u,CHUTE=%u,LORA=%u,GPS=%u,BMP1=%u,BMP2=%u,MPU1=%u,MPU2=%u,FLASH=%u",
